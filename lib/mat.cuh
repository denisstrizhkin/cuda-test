...
/**
 * @brief A templated class representing a fixed-size matrix stored on a CUDA device.
 *
...
public:
    ...
    /**
     * @brief Provides mutable access to an element at the specified row and column.
     *
    ...
    /**
     * @brief Provides constant access to an element at the specified row and column.
     *
     * @param i The row index (0-based).
     * @param j The column index (0-based).
     ...
     *
     * This function computes the dot product (matrix multiplication) of the current
     * matrix (`this`) with `other`. It launches a CUDA kernel to perform the
     * computation on the device and returns a new `Mat` object with the result.
     *
...
private:
    CudaPtr<T> data_; ///< Pointer to the device memory storing the matrix elements.
};
